...
interface MockReadingWrapperProps {
...
function QuestionGroupRenderer({ group, answers, multiAnswers, onAnswer, onMultiAnswer }: { group: QuestionGroup; answers: Record<number, string>; multiAnswers: Record<number, string[]>; onAnswer: (qNum: number, val: string) => void; onMultiAnswer: (qNum: number, val: string[]) => void; }) {
  ...
  const dropdownOptions = useMemo(() => {
    if (group.matchingOptions && group.matchingOptions.length > 0) {
      return group.matchingOptions.map((o) => ({ value: o.letter, label: `${o.letter} - ${o.text}` }));
    }
      ...
      {dropdownOptions && (t === QuestionType.MATCHING_SENTENCE_ENDINGS || t === QuestionType.MATCHING_FEATURES || t === QuestionType.MATCHING_HEADINGS) && (
        <div className="rounded-lg border-[0.5px] border-[#2A3150] bg-[#12172B] p-3 text-xs text-[#94A3B8]">
          {group.matchingOptions?.map((o) => (
            <p key={o.letter} className="py-0.5"><span className="font-bold text-[#F8FAFC]">{o.letter}</span> {o.text}</p>
          ))}
        </div>
      )}

      {hasTemplate && (
        <CompletionTemplate template={group.completionTemplate!} questions={group.questions} answers={answers} onAnswer={onAnswer} />
      )}
...
export default function MockReadingWrapper({ passages, answers, onAnswer }: MockReadingWrapperProps) {
  ...
  if (!passages || passages.length === 0) {
    return <div className="p-4 text-center text-sm text-[#94A3B8]">Loading reading passages...</div>;
  }
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="flex flex-1 flex-col gap-4 lg:flex-row">
        {/* Left pane: passage */}
        <div className="flex-1 overflow-y-auto rounded-xl border-[0.5px] border-[#2A3150] bg-[#1E2540] p-5 shadow-sm max-h-[600px] ielts-scrollbar">
          <div className="mb-6 flex items-start justify-between">
            <div>
              ...
              {currentPassage.subtitle && (
                <p className="mt-1 text-sm italic text-[#94A3B8]">{currentPassage.subtitle}</p>
              )}
            </div>
          </div>

          <div className="prose prose-sm prose-invert max-w-none text-[#CBD5E1]">
            <div dangerouslySetInnerHTML={{ __html: currentPassage.passage }} />

            {currentPassage.passageSections?.map((sec, i) => (
              <div key={i} className="mt-6 flex gap-4">
                <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded bg-[#12172B] font-serif text-lg font-bold text-[#F8FAFC]">
                  {sec.label}
                </div>
        ...
        <div className="flex-1 overflow-y-auto rounded-xl border-[0.5px] border-[#2A3150] bg-[#12172B] p-5 shadow-sm max-h-[600px] ielts-scrollbar">
          <div className="mb-6">
            <h3 className="font-semibold text-[#F8FAFC]">Questions</h3>
            <p className="text-xs text-[#94A3B8]">
              Read the passage on the left and answer the questions below.
            </p>
          </div>
          <div className="space-y-8">
            {currentPassage.questionGroups?.map((group, i) => (
            ...
            {(!currentPassage.questionGroups || currentPassage.questionGroups.length === 0) && (
              <p className="text-sm text-[#64748B]">No questions found for this passage.</p>
            )}
          </div>
        </div>
      </div>
